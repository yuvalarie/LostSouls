...
namespace Game_Flow.ImpactObjects.Scripts.Types
{
    public class TwoBlockHorizontalGridImpactObject : ImpactObjectDecorator
    {
        ...
        public override void UpdateImpact(Vector3 direction)
        {
            ...
            // 1) if a previous move‐tween is still running, skip
            if (_moveTween != null && _moveTween.IsActive() && !_moveTween.IsComplete())
                return;

            // 2) clear the old two‐cell footprint
            ...
            // 6) occupy new cells & get their world‐center
            Vector3 worldCenter = _grid.MarkOccupied(Mono, targetCells);

            // 7) animate into place, then commit
